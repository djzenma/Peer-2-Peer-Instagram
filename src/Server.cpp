...
void Server::updateCount(int image_id, int num_views){
    std::string image_path =(std::string)PATH   + "/images/requested/" + to_string(image_id) + ".jpg";
    std::string output = stega_decode(image_path);
    int sep_index = output.find(',');
    std::string tempPath =(std::string)PATH   + "/images/stego/temp.jpg";
...
int Server::serveRequest(){
    ...
    Thread * thrd = new Thread(false , true ,  reqNum ,image_id,false, 4040 , toConnectIp , toConnectPort, 0 ); //server thread
