 ...
 */
int Communication::getImage(bool & single , Message &m, const int receivingPort, std::string receiverIP , std::string directory) {
    ...
    int buffersize = 0, recv_size = 0, size = 0, read_size = -1, write_size, packet_index = 1, status;
    ...
    do {
        status = recvfrom(socketfd,&size, sizeof(int),0,(struct sockaddr*)&serverAddr, &addr_size);
    } while (status < 0);
    ...
    if (recieved_msg.length() >= size && size != 0) {
        ...
        std::cout<<"Owner Name: "<<m.getOwnerName()<<"\n";
        std::cout<<"Owner Image Id: "<<m.getImageId()<<"\n";
        ...
        std::string temp_loc = "./../images/"+ directory + std::to_string(m.getImageId()) +".jpg";
